...
public class OrderServices implements OrderService {
	...
	/*
	 * it is used to calculate the total price for the order
	 */
	...
	public ResponseEntity<ResponseStructure<Orders>> updateOrders(int order_Id, OrderDto orderDto) {
		...
		if (orderDto != null && order != null) {
			...
			existingItems.removeIf(existingItem -> updatedItems.stream()
					.noneMatch(updatedItem -> existingItem.getItemName().equalsIgnoreCase(updatedItem.getItemName())));
	...
	/*
	 * to get all the orders present in the order table
	 */
	@Override
